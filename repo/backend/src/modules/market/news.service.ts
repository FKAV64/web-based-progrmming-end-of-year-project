    ...
    return this.cache.wrap(
      ...
      async () => {
        try {
          const [coindesk, cointelegraph] = await Promise.all(
            [
              ...
              p.catch((err: unknown) => {
                ...
                this.logger.warn(`Failed to fetch RSS: ${msg}`);
                return { items: [] as never[] };
              }),
            ),
          );

          const items = [...coindesk.items, ...cointelegraph.items].map(
            (item) => ({
              title: item.title,
              link: item.link,
              pubDate: item.pubDate,
              source: item.link?.includes('coindesk')
                ? 'CoinDesk'
                : 'CoinTelegraph',
              sentiment: this.guessSentiment(
          ...
          this.logger.error(`News fetch failed: ${msg}`);
          return [];
        }
      },
      15 * 60_000,
