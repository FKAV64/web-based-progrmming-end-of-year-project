...
describe('BinanceRestService', () => {
  ...
  it('should map Binance response correctly', async () => {
    globalFetchMock.mockResolvedValue({
      ok: true,
      json: async () => [
        [1609459200000, "28923.63000000", "29031.34000000", "28690.17000000", "28995.13000000"]
      ],
    });
  ...
  it('should throw 400 on invalid interval', async () => {
    await expect(service.getKlines('BTCUSDT', '7m')).rejects.toThrow(BadRequestException);
  });

  it('should throw 400 on limit out of bounds', async () => {
    await expect(service.getKlines('BTCUSDT', '1h', 1001)).rejects.toThrow(BadRequestException);
  });
});
